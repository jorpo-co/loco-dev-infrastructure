...
teardown:
  ...
  @{{ SCRIPTS_DIR }}/certs.sh uninstall
...
# open a specifid infrastructure service in the browser
open service:
  @echo "Opening http://{{ service }}.loco ..."
  @open http://{{ service }}.loco
...
certs-init:
  @{{ SCRIPTS_DIR }}/certs.sh install

# generate TLS certs for infra's own services (registry, dashboard)
certs-infra:
  @{{ SCRIPTS_DIR }}/certs.sh infra
...
ps:
  ...
  @echo "══════════════════════════════════════════════"
  @ls -1 {{ PROJECT_DIR }}/etc/traefik/configs/*.yml 2>/dev/null | sed 's/^/  /' || echo "  (none)"
  @echo ""
  @echo "══════════════════════════════════════════════"
...
env:
  ...
  @echo "  Providers:    {{ PROJECT_DIR }}/etc/traefik/configs/"
